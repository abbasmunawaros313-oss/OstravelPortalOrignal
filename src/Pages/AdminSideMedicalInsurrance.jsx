...
import { FaEdit, FaSpinner, FaFilePdf, FaSearch, FaFilter, FaCalendarAlt, FaCalendarWeek, FaCalendarDay, FaUser, FaBuilding, FaPassport, FaIdCard, FaGlobe, FaPhone, FaDollarSign, FaEye, FaSortDown, FaSortUp, FaTrash } from "react-icons/fa";
import { MdClose } from "react-icons/md";
import { BiCalculator } from "react-icons/bi";

// Assuming AdminNavbar is a valid component
import AdminNavbar from "../Components/AdminNavbar";

export default function AdminSideMedicalInsurrance() {
    ...
    const formFields = useMemo(() => [
        ...
        { label: "Total Received Amount", name: "totalReceivedAmount", type: "number", required: true, icon: FaDollarSign },
        { label: "Total Payable Amount", name: "totalPayableAmount", type: "number", required: true, icon: FaDollarSign },
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            setBookings(data);
            setIsLoading(false);
            const uniqueCompanies = [...new Set(data.map(b => b.NameofCompany))].sort();
    ...
    useEffect(() => {
        let tempBookings = selectedCompany ? bookings.filter(b => b.NameofCompany === selectedCompany) : bookings;
        ...
        if (filterPeriod === "today") {
            ...
            tempBookings = tempBookings.filter(b => {
                const bookingDate = b.createdAt?.toDate?.() || (b.createdAt ? new Date(b.createdAt) : null);
                return bookingDate && bookingDate >= new Date(startDate) && bookingDate <= new Date(endDate);
            });
        }
        ...
        // Sorting
        const sortedBookings = [...tempBookings].sort((a, b) => {
    ...
    }, [bookings, searchTerm, filterPeriod, startDate, endDate, selectedCompany, sortBy, sortOrder]);
    ...
    const handleDeleteClick = async (id) => {
        if (window.confirm("Are you sure you want to delete this record? This action cannot be undone.")) {
            try {
                const docRef = doc(db, "medical_insurance", id);
    ...
    const handleSaveEdit = async (dataToUpdate) => {
        ...
        try {
            ...
            const updatedData = {
                ...dataToUpdate,
                totalProfit: Number(dataToUpdate.totalReceivedAmount) - Number(dataToUpdate.totalPayableAmount)
            };
    ...
    const EditModal = ({ booking, formFields, onClose, onSave, isSaving }) => {
        ...
        const handleSubmit = async (e) => {
            ...
            const fieldsToConvert = ["age", "noOfdays", "totalReceivedAmount", "totalPayableAmount"];
    ...
    return (
        ...
        <div className="relative min-h-screen bg-black overflow-hidden font-sans text-gray-100 p-8">
            ...
            {/* Main Content */}
            <div className="relative z-20 w-full max-w-7xl mx-auto pt-24">
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-gray-700 shadow-2xl rounded-xl p-4 flex flex-col md:flex-row items-center gap-4 mb-8">
                    <div className="flex-grow flex flex-col md:flex-row items-center gap-4">
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto">
                            <input
                                ...
                                className="w-full md:w-auto px-4 py-2 rounded-lg bg-gray-800/50 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div className="flex flex-wrap gap-2 justify-center md:justify-end">
                        <button
                            onClick={() => { setFilterPeriod("all-time"); setStartDate(""); setEndDate(""); }}
                        ...
                        <button
                            onClick={() => { setFilterPeriod("today"); setStartDate(""); setEndDate(""); }}
                        ...
                        <button
                            onClick={() => { setFilterPeriod("this-week"); setStartDate(""); setEndDate(""); }}
                        ...
                        <button
                            onClick={() => { setFilterPeriod("this-month"); setStartDate(""); setEndDate(""); }}
                ...
                <div className="mb-8">
                    <h2 className="text-2xl font-bold text-white mb-4">Bookings by Company</h2>
                    <div className="flex flex-wrap gap-4 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900">
                        {companies.map(company => (
                            <button
                                key={company}
                                onClick={() => setSelectedCompany(selectedCompany === company ? null : company)}
                                className={`flex-shrink-0 px-6 py-3 rounded-xl border-2 transition-all duration-300 transform hover:scale-105 ${
                ...
                <div className="bg-white/5 backdrop-blur-xl border border-gray-700 shadow-2xl rounded-3xl p-6 md:p-10 animate-fade-in-up">
                    <h2 className="text-2xl font-bold text-white mb-6">{selectedCompany ? `${selectedCompany} Records` : "All Records"}</h2>
                    {isLoading ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-700 table-auto">
                                <thead className="bg-gray-800/50">
                                    <tr>
                                        {['#', 'Company', 'Insured Name', 'Passport No.', 'Travel Country', 'Rec. Amount', 'Pay. Amount', 'Profit', 'Actions'].map(header => (
                                            <th
                                                key={header}
                                                ...
                                                onClick={() => {
                                                    const sortableHeaders = {
                                                        ...
                                                        'Insured Name': 'NameofInsured',
                                                        'Rec. Amount': 'totalReceivedAmount',
                                                        'Pay. Amount': 'totalPayableAmount',
                                                        'Profit': 'totalProfit',
                                                    };
                                                    const sortKey = sortableHeaders[header] || null;
                                                    if (sortKey) handleSort(sortKey);
                                                }}
                                            >
                                                <div className="flex items-center gap-1">
                                                    {header}
                                                    {sortBy === (header === 'Company' ? 'NameofCompany' : header === 'Insured Name' ? 'NameofInsured' : header === 'Rec. Amount' ? 'totalReceivedAmount' : header === 'Pay. Amount' ? 'totalPayableAmount' : header === 'Profit' ? 'totalProfit' : '') && (
                                                        sortOrder === "asc" ? <FaSortUp className="text-blue-400" /> : <FaSortDown className="text-blue-400" />
                                                    )}
                                                </div>
                                            </th>
                                        ))}
                                        ...
                                        <tr key={booking.id} className="hover:bg-white/10 transition-colors duration-200">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div className="flex space-x-2">
                                                    ...
                                                    <button onClick={() => handleDeleteClick(booking.id)} className="text-red-400 hover:text-red-300 transition-colors p-2 rounded-full hover:bg-white/10" aria-label="Delete booking">
                                                        <FaTrash />
                                                    </button>
                                                </div>
                                            </td>
